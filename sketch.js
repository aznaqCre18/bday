...
var font;
var foto, bgStatic;
var txt = "Barakallah Fii Umrik";
var name = "Dyah Puji Astuti";
var sfxPop;

function preload() {
  foto = loadImage("assets/img/dy.png");
...
function draw() {
  ...
  // foto
  translate(
    imgX + (-mouseX + width / 2) / 30,
    imgY + (-mouseY + height / 2) / 30
  );
  imageMode(CENTER);
  ...
  text(
    name,
    imgX + (mouseX - width / 2) / 30,
    imgY + 200 + (mouseY - height / 2) / 30
  );
  // teks hbd
  text(
    txt,
    imgX + (mouseX - width / 2) / 30,
    imgY - 150 + (mouseY - height / 2) / 30
  );
  pop();
